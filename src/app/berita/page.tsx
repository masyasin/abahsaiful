...
const allNews = [
  { title: "Rapat Komisi II Bahas Program Infrastruktur Jalan", category: "Komisi II", date: "2026-06-20", excerpt: "Komisi II DPRD Kota Tangerang menggelar rapat dengar pendapat dengan Dinas PU terkait program perbaikan jalan." },
  { title: "Saiful Milah Resmikan Posyandu di Ciledug", category: "Fraksi", date: "2026-06-15", excerpt: "Peresmian Posyandu baru di Kelurahan Sudimara, Ciledug sebagai bagian dari program kesehatan masyarakat." },
  { title: "DPRD Setujui Raperda Pendidikan Inklusif", category: "Komisi II", date: "2026-06-10", excerpt: "DPRD Kota Tangerang menyetujui Raperda tentang Pendidikan Inklusif untuk anak berkebutuhan khusus." },
  { title: "Kunjungan Kerja Fraksi Golkar ke Larangan", category: "Fraksi", date: "2026-06-05", excerpt: "Fraksi Golkar DPRD Kota Tangerang melakukan kunjungan kerja ke Kecamatan Larangan untuk menyerap aspirasi." },
  { title: "Program Beasiswa 2026 Dibuka", category: "Fraksi", date: "2026-05-28", excerpt: "Pendaftaran program Beasiswa Pendidikan resmi dibuka untuk siswa SMA/SMK di Kota Tangerang." },
  { title: "Fraksi Golkar Kaji Raperda Perlindungan Buruh", category: "Fraksi", date: "2026-05-20", excerpt: "Fraksi Golkar sedang mengkaji Raperda tentang Perlindungan Buruh di Kota Tangerang." },
];
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-slate-50/60 py-16">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-12 overflow-hidden rounded-2xl border border-amber-200/60 bg-white shadow-md transition-all duration-300 hover:shadow-lg">
            <div className="grid md:grid-cols-2">
              <div className="relative flex items-center bg-gradient-to-br from-yellow-400 via-amber-300 to-yellow-500 p-8 text-slate-900 sm:p-10">
                <div className="absolute top-0 right-0 h-32 w-32 rounded-full bg-white/10 blur-[40px] pointer-events-none" />
                <div>
                  <Badge className="mb-4 bg-slate-900 text-yellow-300 font-bold border border-slate-950/20 px-3 py-1">
                    {featuredNews.category}
                  </Badge>
                  <h2 className="text-xl font-black leading-tight tracking-tight sm:text-2xl lg:text-3xl text-slate-950">
                    {featuredNews.title}
                  </h2>
                  <p className="mt-4 text-sm leading-relaxed text-amber-950/85 font-medium">
                    {featuredNews.excerpt}
                  </p>
                  <div className="mt-6 flex flex-wrap items-center gap-4 text-xs font-bold text-slate-800">
                    <span className="flex items-center gap-1.5 text-amber-950">
                      <Calendar size={14} />
                      {featuredNews.date}
                    </span>
                  </div>
                  <button className="mt-6 inline-flex items-center gap-1.5 text-sm font-black text-slate-950 hover:underline transition-colors duration-200 cursor-pointer">
                    Baca Selengkapnya <ArrowRight size={14} />
                  </button>
                </div>
              </div>
              <div className="flex items-center justify-center bg-amber-50/10 p-8">
                <div className="flex h-48 w-48 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500/10 to-yellow-500/10 border border-amber-500/15 text-6xl shadow-inner shadow-amber-500/5">
                  📰
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((news, i) => (
              <div
                key={i}
                className="group rounded-2xl border border-amber-200/60 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-amber-500/35 hover:shadow-md"
              >
                <div className="mb-3">
                  <Badge
                ...
                <p className="mt-2 text-sm text-slate-500 leading-relaxed font-medium line-clamp-3">{news.excerpt}</p>
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400 font-semibold">
                  ...
                  <button className="font-bold text-amber-650 hover:text-amber-550 transition-colors duration-200 cursor-pointer">
                    Baca Berita &rarr;
                  </button>
                </div>
              </div>
